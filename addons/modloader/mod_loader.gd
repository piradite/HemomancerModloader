...
func _check_for_game_instance():
	if is_instance_valid(Global.game) and is_instance_valid(Global.game.player) and Global.game.player.is_node_ready():
		if not game_ready:
			gameready = true
			await _on_game_started()
			game_ready.emit()
	else:
		gameready = false
...
func _recalculate_all_stats():
	if is_instance_valid(Global.game) and is_instance_valid(Global.game.player):
		...
		Global.game.player.print_stats("---" + " MODDED STATS (After ModLoad) ---")
	else:
		pass
...
func _generate_and_apply_hook_script(node: Node):
	var node_path = node.get_path()
	if not hooks.has(node_path): return
	...
	for method_name in mod_methods:
		...
		if replacement:
			...
			var can_call_super = original_script.has_method(method_name) or method_name.begins_with("_")
			if can_call_super:
				if is_void:
