...
func _ready():
	...
	if version == "" or version == "0.0.0":
		...
		if main_menu_scene:
			...
			var version_label = main_menu_instance.get_node_or_null("Version")
			if version_label:
				var regex = RegEx.new()
				regex.compile("\\d+\\.\\d+\\.\\d+")
				var result = regex.search(version_label.text)
				if result:
...
func _on_settings_ready():
	version = ProjectSettings.get_setting("application/config/version", "0.0.0")
	if version == "" or version == "0.0.0":
		var main_menu_scene = get_tree().get_root().find_child("MainMenu", true, false)
		if main_menu_scene:
			var version_label = main_menu_scene.get_node_or_null("Version")
			if version_label:
				var regex = RegEx.new()
				regex.compile("\\d+\\.\\d+\\.\\d+")
				var result = regex.search(version_label.text)
	...
	for mod_dir in MODS_DIRS:
		...
		file_watcher.start_watching(mod_dir)
...
func _inject_mods_button(main_menu_node):
	...
	var button_scene
	if ResourceLoader.exists("res://ui/controls/button/CustomButton.tscn"):
		button_scene = load("res://ui/controls/button/CustomButton.tscn")
	elif ResourceLoader.exists("res://ui/hud/controls/button/CustomButton.tscn"):
		button_scene = load("res://ui/hud/controls/button/CustomButton.tscn")
	else:
		push_warning("Could not find CustomButton.tscn for mod loader.")
		return
...
func _on_game_started():
	...
	for child in get_children():
		if child is ModBase and child.mod and child.mod.enabled:
			await child._on_game_started()
				...
				if dir.current_is_dir() and not file_name.begins_with("."):
					...
					if FileAccess.file_exists(manifest_path):
						...
						if not manifest.is_empty():
							...
							if mod_game_version != "*" and mod_game_version.strip_edges() != version.strip_edges():
								mod.version_mismatch = true

							var script_path = manifest.get("main_script")
							if script_path:
								var full_script_path = mod.path.path_join(script_path)
								if FileAccess.file_exists(full_script_path):
									var script_content = FileAccess.open(full_script_path, FileAccess.READ).get_as_text()
	...
	if file:
		...
		if error == OK:
			...
			if typeof(data) == TYPE_DICTIONARY:
				...
				ModNotifications.show_notification("Invalid manifest format for %s. Expected a Dictionary." % path)
				return {}
		else:
			ModNotifications.show_notification("Failed to parse manifest for %s. Error: %s" % [path, json.get_error_message()])
			return {}
	return {}
...
func _resolve_dependencies():
	...
	for mod_id in graph:
		for dependency in graph[mod_id]:
			if not mods.has(dependency):
				ModNotifications.show_notification("Mod '%s' is missing dependency '%s'" % [mod_id, dependency])
				continue
	...
	if order.size() < mods.size():
		ModNotifications.show_notification("Circular dependency detected in mods. Loading in default order.")
		order = mods.keys()
...
func _load_mod(mod: Mod):
	...
	if script_path:
		...
		if err != OK:
			ModNotifications.show_notification("Mod '%s' crashed during loading. Disabling mod." % mod.get_name())
			mod.enabled = false
	...
	if FileAccess.file_exists(pack_path):
		if not ProjectSettings.load_resource_pack(pack_path):
			ModNotifications.show_notification("Failed to load assets for mod: %s" % mod.get_name())
...
func enable_mod(mod_id: String):
	...
	if not mod.enabled:
		...
		if _game_started_triggered and is_instance_valid(mod.instance) and mod.instance is ModBase:
			mod.instance._on_game_started()
